...
sg4::CommPtr Actions::comm_file_async(Job* j, const std::string& filename, const std::string& src_site, const std::string& dst_site, const unsigned long long& size, std::unique_ptr<DispatcherPlugin>& dispatcher, FileTransferMode mode)
{
    ...
    std::string transfer_type = (mode == FileTransferMode::MOVE) ? "MOVE" : "COPY";
    comm_activity->set_name("Comm_transfer_File_" + filename + "_for_Job_" + std::to_string(j->jobid)+"_from_"+src_site+"_to_"+dst_site+"_"+transfer_type);
    ...
    comm_activity->on_this_completion_cb([filename,size,src_site,dst_site,j, &dispatcher, mode](simgrid::s4u::Comm const& co) {
        // Create file on destination site
        CGSim::FileManager::create(filename,size,dst_site);
        
        // If MOVE operation, remove file from source site after successful transfer
        if (mode == FileTransferMode::MOVE) {
            CGSim::FileManager::remove(filename, src_site);
        }
